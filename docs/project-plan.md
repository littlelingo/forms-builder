...
- 2026-05-14: Behavior graph discovery shipped on `main`. Spec: [docs/superpowers/specs/2026-05-14-behavior-graph-discovery-design.md](/Users/clint/Workspace/forms-builder/docs/superpowers/specs/2026-05-14-behavior-graph-discovery-design.md). Plan: [docs/superpowers/plans/2026-05-14-behavior-graph-discovery.md](/Users/clint/Workspace/forms-builder/docs/superpowers/plans/2026-05-14-behavior-graph-discovery.md). Three discovery features layered onto the existing BehaviorWorkspace card-graph: (1) payload-fields popover on listener trigger nodes via new `EventPayloadBadge` hover chip + `PayloadFieldsPopover`; (2) cross-step ref badges (`CrossStepRefBadge`) + distinct edge tone (new `BehaviorEdgeLabel` `tone="crossStep"` with `graph-edge-cross-step` class for E2E selectors); (3) reverse-index badges (`ReverseIndexBadge`) on rule trigger/effect field nodes, counting listeners referencing the field, click handler wires to a new `handleOpenReverseIndex` in App that builds an `AuthoringSelection` for the node and opens the existing inspector path. Composer-side: `ActionEditor`'s `set_field_value` static-value input swapped to `PayloadFieldAutocomplete` (datalist of `{{event.payload.X}}` options driven by listener's event payload schema). Cleanup: dropped orphaned `builder*Options` props from `ActionEditor` (still used by `RuntimeReactionProperties` via `BehaviorComposer` — those kept). Pure helpers own derivation: new `apps/web/src/lib/payload-schema-helpers.ts` (`listPayloadFieldsForEventType` resolves core → form-level → node-level event sources; `isCrossStepReference` + `collectCrossStepRefsForListener`) with 7 unit tests, plus `payload-field-autocomplete-logic.ts` with 3 tests. Plan was adapted from spec assumption: graph is card-based, not SVG — edges are pills and cross-step styling adjusts pill tone instead of an SVG stroke. No engine changes; no schema changes. Gates: typecheck:web, build:schema, build:runtime, build:web, runtime tests **110/110**, pytest **99/99**, E2E **3/3** (phase3 + test-panel + walkthrough unchanged), payload-schema-helpers **7/7**, payload-field-autocomplete-logic **3/3**, format:check clean.
- 2026-05-14: Field-rules authoring slice shipped on `main`. Spec [docs/superpowers/specs/2026-05-14-field-rules-authoring-design.md](/Users/clint/Workspace/forms-builder/docs/superpowers/specs/2026-05-14-field-rules-authoring-design.md). Plan [docs/superpowers/plans/2026-05-14-field-rules-authoring.md](/Users/clint/Workspace/forms-builder/docs/superpowers/plans/2026-05-14-field-rules-authoring.md). First guided UX for visibility/required rules: new `FieldRuleWizard` modal (5-input wizard — effect / affected / trigger / operator / value) produces runtime listeners under a constrained shape (`field.change` event + single atom condition + single `show_node` / `hide_node` / `mark_required` / `mark_optional` action). Two entry points feed the same wizard: `FieldRulesList` in the field inspector (rules affecting the selected field, with conflict warnings for opposing effects under the same condition) + `FieldRulesTriggers` in `BehaviorWorkspace` (rules the selected field triggers). Pure helpers in `apps/web/src/lib/field-rule-helpers.ts` (`isFieldRuleListener` + `decodeFieldRule` + `encodeFieldRule` + `findRulesAffectingField` + `findRulesTriggeredByField` + `detectFieldRuleConflicts`) with **15** unit tests. `value-picker.tsx` adapts the expected-value input to the trigger field's semantic type (choice options → dropdown, checkbox → boolean select, number → numeric input, plain → text, `exists` → hint text). No schema or engine change — the rule shape is a structural convention over `RuntimeListenerDefinition`. Path-branching authoring deferred to next slice. Gates: typecheck:web, build:schema, build:runtime, build:web, runtime tests **110/110**, pytest **99/99**, E2E **5/5** (phase3 + test-panel + walkthrough + library-modal + new field-rules), field-rule-helpers **15/15**, format:check clean.
